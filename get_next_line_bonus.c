...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/14 18:34:43 by wrolanda          #+#    #+#             */
/*   Updated: 2021/12/14 18:34:43 by wrolanda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || BUFFER_SIZE >= INT_MAX - 1)
		return (0);
	st_line[fd] = ft_saves_read(fd, st_line[fd]);
	if (!st_line[fd])
		return (NULL);
	res_line = ft_get_line(st_line[fd]);
	st_line[fd] = ft_rest_st_line(st_line[fd]);
	return (res_line);
}
